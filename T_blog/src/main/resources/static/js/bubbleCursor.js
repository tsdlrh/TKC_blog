...
function bubbleCursor(options) {
  ...
  // 绑定特定事件到鼠标操作
  function bindEvents() {
    element.addEventListener("mousemove", onMouseMove)//鼠标移动
    element.addEventListener("touchmove", onTouchMove)//移动端移动手指
    element.addEventListener("touchstart", onTouchMove)//移动端移动手指
    window.addEventListener("resize", onWindowResize)//窗口尺寸变化
  }
